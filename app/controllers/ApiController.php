...
class ApiController extends Controller {
    ...
    public function __construct() {
       ...
       $publicRoutes = [
            ...
            'forgotPassword',
            'resetPassword',
            'create',           // register (kung may public register ka)
            'listCarsPaginated', // public car listing
            'getBookedDates',
            'compareCars',
            'listDealers'
        ];
        ...
        if (!in_array($currentRoute, $publicRoutes)) {
            if (empty($_SESSION['user_id'])) {
                $this->api->respond_error('Unauthorized. Please login first.', 401);
                exit;
            }

            // Ilagay sa $this->user ang totoong user data (server-side na!)
            $this->user = [
                'id'        => $_SESSION['user_id'],
    ...
    private function getCurrentRoute() {
        ...
        foreach ($trace as $t) {
            if (isset($t['object']) && $t['object'] === $this && isset($t['function'])) {
                return $t['function'];
            }
        }
        return 'unknown';
    ...
    private function getCurrentUser(): array
        {
            // Kung may naka-login na user (from session) → ibalik ang totoong data
            if ($this->user !== null) {
                return $this->user;
            }

            // Kung walang login (public route lang) → guest mode
            return [
                'id'        => null,
                'role'      => 'guest',     // ← HINDI NA "admin" o "buyer" — GUEST LANG!
                'dealer_id' => null,
                'name'      => 'Guest',
                'email'     => null
            ];
...
// =====================================================================================================================
    ...
    $this->api->require_post();  // mas consistent sa iba
    $input = $this->api->body();
    $email = $input['email'] ?? '';
    ...
    if ($user && password_verify($password, $user['password_hash'])) {
        ...
        session_regenerate_id(true); // anti session fixation
            ...
            $_SESSION['dealer_id']   = $user['dealer_id'] ?? null;
    ...
    public function create() {
        ...
        $dealer_id = $input['dealer_id'] ?? null;  // default dealer_id is null
    ...
    public function forgotPassword()
    {
        ...
        if (!$user) {
            return $this->api->respond(['message' => $message]);
        }
        ...
        // Send email (gamit ang function mo na nasa class)
    ...
    public function resetPassword()
    {
        ...
        if (!$email || !$token || !$password || $password !== $confirm) {
            return $this->api->respond_error('All fields are required and passwords must match', 400);
        }
        ...
        $this->db->raw("UPDATE users SET password_hash = ? WHERE email = ?", [$hashed, $email]);
...
// =========================================================================================================
    ...
    public function listCars() {
        $user = $this->getCurrentUser(); // safe na talaga ‘to!
...
// ===========================================================================================
    ...
    public function downloadFile($filename)
    {
        ...
        if (!file_exists($filePath)) {
            return $this->api->respond_error('File not found.', 404);
        }
